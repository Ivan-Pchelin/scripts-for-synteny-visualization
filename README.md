...
3. **rename_fasta.py** can be used to rename the sequences in the file. The matches between accession numbers and names are written manually to rename.txt. The script is launched from the folder containing the file with sequences and the file rename.txt. Rename.txt is formatted in the following way:
